...
| FR2 — real messaging, not shared memory | ⚠️ simplified | See **"The one honest simplification"** below |
...
## The one honest simplification (FR2)
...
This build runs all robots **in one Node.js process** for reliability within
a hackathon build window, but enforces the *behavioral* property FR2 is
really testing for: no robot's code ever reads another robot's fields
directly. Every interaction — a bid, a movement intent, a yield — goes
through `lib/simulation/messageBus.ts`'s publish/subscribe interface, the
same discipline you'd need with real sockets. `auction.ts` and `robot.ts`
don't know or care whether the bus is in-process or over UDP.

**If a judge pushes on this** (and a good one will): the honest answer is
"the coordination *protocol* is real and decentralized; the transport is
simulated in-process for build-time reliability, and is a one-file swap —
not a redesign — to make literal," and then show them
`lib/simulation/messageBus.ts`. That file is small on purpose, specifically
so this claim is checkable in under a minute.
...
6. **Close** — what Round 2 adds: real multi-process transport, a trained
   detection model, physical hardware.
...
- Swap `messageBus.ts` for real UDP or ROS2 DDS transport across N actual
  processes (or containers) — proves FR2 literally, not just behaviorally.
...
- Add simulated packet loss / latency injection with a live degradation
  graph (PRD nice-to-have).
- Put the agent loop on a real Raspberry Pi or Jetson Nano chassis.
...
reset without restarting the server), and this README.
